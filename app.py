...
def _cleanup_stale_session_state(current_lead_ids: set[str]):
    ...
    for key in st.session_state.keys():
        # Check for stage_history_ and notes_ prefixed keys
        if key.startswith("stage_history_"):
            lead_id = key[len("stage_history_"):]
            if lead_id not in current_lead_ids:
                keys_to_remove.append(key)
        elif key.startswith("stage_history_error_"):
            lead_id = key[len("stage_history_error_"):]
            if lead_id not in current_lead_ids:
                keys_to_remove.append(key)
        elif key.startswith("notes_"):
            lead_id = key[len("notes_"):]
            if lead_id not in current_lead_ids:
                keys_to_remove.append(key)
